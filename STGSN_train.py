...
if __name__ == "__main__":
    ...
    structural = [1, 256, 256, feats[0].shape[1]]
    ...
    for epoch in range(args.epochs):
        ...
        opt.zero_grad()
        loss = model.get_loss(sup_list, feat_list, col_sup_tnr, feat_list[-1], feat.shape[0],gnd_tnr)
        loss.backward()
        opt.step()
        epoch_loss.append(loss.item())
        ...
        model.eval()
        emb = model(sup_list, feat_list, col_sup_tnr, feat_list[-1], feat.shape[0]).detach().cpu().numpy()
        ...
        if epoch_auc_val > best_epoch_val:
            ...
            torch.save(model.state_dict(), "./model_checkpoints/STGSN/model_{}.pt".format(args.dataset))
            patient = 0
        else:
            patient += 1
    ...
    # Test Best Model
    model.load_state_dict(torch.load("./model_checkpoints/STGSN/model_{}.pt".format(args.dataset)))
    model.eval()
    emb = model(sup_list, feat_list, col_sup_tnr,feat_list[-1], feat.shape[0]).detach().cpu().numpy()
