...
wandb.init(project="dyngraph2vec", entity="houjinlin")
...
if __name__ == "__main__":
    ...
    structural = [feats[0].shape[1], int(args.structural_layer_config)]
    temp = [int(args.structural_layer_config), int(args.temporal_layer_config), int(args.temporal_layer_config)]
    dec = [int(args.temporal_layer_config),feats[0].shape[1]]
    model = dyngraph2vec(structural, temp, dec, args.spatial_drop).to(device)
    # model = DySAT(args, 1, args.time_steps).to(device)
    opt = torch.optim.Adam(model.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay)

    sup_list = [torch.Tensor(sp.coo_matrix(a, dtype=np.float32).todense()).to(device) for a in adjs[:-1]]
    ...
    for epoch in range(args.epochs):
        ...
        opt.zero_grad()
        loss = model.get_loss(sup_list, gnd_tnr)
        loss.backward()
        opt.step()
        epoch_loss.append(loss.item())
        ...
        if epoch_auc_val > best_epoch_val:
            ...
            torch.save(model.state_dict(), "./model_checkpoints/EvolveGCN/model_{}.pt".format(args.dataset))
            patient = 0
        else:
            patient += 1
    ...
    # Test Best Model
    # model.load_state_dict(torch.load("./model_checkpoints/dyngraph2vec/model_{}.pt".format(args.dataset)))
    # model.eval()
    # emb = model(sup_list).detach().cpu().numpy()
    ...
    print("Best Test AUC = {:.3f} Best AP = {:.3f} ".format(best_epoch_test,best_epoch_ap))
    wandb.log({"Best User AUC":best_epoch_test,"Best User AP":best_epoch_ap})
