...
from models.egcn import EvolveGCN
import torch
...
wandb.init(project="EvolveGCN", entity="houjinlin")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--egcn_type',type=str, nargs='?', default='EGCNH')# EGCNH / EGCNO
    ...
    model = EvolveGCN(int(feats[0].shape[1]), 256,int(feats[0].shape[1]), args.egcn_type).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        opt.zero_grad()
        loss = model.get_loss(sup_list, feat_list, gnd_tnr)
        loss.backward()
        opt.step()
        epoch_loss.append(loss.item())
        ...
        model.eval()
        emb = model(sup_list, feat_list).detach().cpu().numpy()
    ...
    # Test Best Model
    model.load_state_dict(torch.load("./model_checkpoints/EvolveGCN/model_{}.pt".format(args.dataset)))
    model.eval()
    emb = model(sup_list, feat_list).detach().cpu().numpy()
