...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        if epoch_auc_val > best_epoch_val:
            ...
            torch.save(model.state_dict(), "./model_checkpoints/gru_gcn/model_{}.pt".format(args.dataset))
            patient = 0
        else:
            patient += 1
        ...
        print("Epoch {:<3},  Loss = {:.3f}, Val AUC {:.4f} Test AUC {:.4f} Test AP {:.3f}".format(epoch, 
                                                                np.mean(epoch_loss),
                                                                epoch_auc_val, 
                                                                epoch_auc_test,
                                                                test_ap))
    ...
    # Test Best Model
    model.load_state_dict(torch.load("./model_checkpoints/gru_gcn/model_{}.pt".format(args.dataset)))
    model.eval()
    emb = model(sups, feats).detach().cpu().numpy()
    ...
    print("Best Test AUC = {:.4f} Best AP = {:.3f} ".format(auc_test,ap))
