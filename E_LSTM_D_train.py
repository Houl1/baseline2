...
wandb.init(project="STGSN", entity="houjinlin")
...
if __name__ == "__main__":
    ...
    model = STGSN(structural, args.spatial_drop).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        opt.zero_grad()
        loss = model.get_loss(sup_list, feat_list, col_sup_tnr, feat, feat.shape[0],gnd_tnr)
        loss.backward()
        opt.step()
        epoch_loss.append(loss.item())
        ...
        model.eval()
        emb = model(sup_list, feat_list, col_sup_tnr, feat, feat.shape[0]).detach().cpu().numpy()
        ...
        if epoch_auc_val > best_epoch_val:
            ...
            torch.save(model.state_dict(), "./model_checkpoints/dyngraph2vec/model.pt")
            patient = 0
        else:
            patient += 1
        ...
        print("Epoch {:<3},  Loss = {:.3f}, Val AUC {:.3f} Test AUC {:.3f}".format(epoch, 
                                                                ...
                                                                epoch_auc_val, 
                                                                epoch_auc_test))
        wandb.log({"Epoch": epoch,"loss":np.mean(epoch_loss),"Val AUC":epoch_auc_val,"Test AUC":epoch_auc_test})                                                        
    # Test Best Model
    model.load_state_dict(torch.load("./model_checkpoints/dyngraph2vec/model.pt"))
    model.eval()
    emb = model(sup_list, feat_list, col_sup_tnr, feat, feat.shape[0]).detach().cpu().numpy()
    val_results, test_results, _, _ = evaluate_classifier(train_edges_pos,
                                                        train_edges_neg,
    ...
    wandb.log({"Best Test AUC":auc_test})
